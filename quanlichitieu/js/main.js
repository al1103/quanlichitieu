...
document.addEventListener("layoutLoaded", () => {
  ...
  apiFetch("/api/auth/me")
    .then((latestUser) => {
      ...
      if (sidebarEmailEl && latestUser.email)
        sidebarEmailEl.textContent = latestUser.email;

      // Cập nhật giao diện menu dựa trên role
      const isAdmin =
        latestUser.role && latestUser.role.toUpperCase() === "ADMIN";
    ...
    .catch(() => {
      /* token hết hạn đã được apiFetch xử lý (đá về login) */
    });
  ...
  const buildTxRowsHtml = (transactions) => {
    ...
    window.loadedTransactionsMap = window.loadedTransactionsMap || {};
    ...
    transactions.forEach((tx) => {
      ...
      const amountStr =
        tx.type === "THU"
          ...
          : `-${formatCurrency(tx.amount)}`;
      const amountClass =
        tx.type === "THU" ? "amount-positive" : "amount-negative";
      ...
      desktopHtml += `
        <tr class="hover:bg-slate-50 transition-colors group">
          ...
          <td class="px-5 py-3 text-right">
            <button onclick="window.openEditTxModal('${tx.id}')" class="text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors mr-3">
              Sửa
            </button>
            <button onclick="window.deleteTx('${tx.id}')" class="text-sm font-medium text-rose-600 hover:text-rose-800 transition-colors">
              Xóa
            </button>
          </td>
        </tr>
      ...
      mobileHtml += `
        <div class="px-4 py-3 flex justify-between items-center bg-white hover:bg-slate-50">
          ...
          <div class="flex flex-col items-end gap-1">
            <div class="text-sm font-medium whitespace-nowrap ${amountClass}">${amountStr}</div>
            <div class="flex gap-2">
              <button onclick="window.openEditTxModal('${tx.id}')" class="text-xs font-medium text-blue-600 hover:text-blue-800 transition-colors">Sửa</button>
              <button onclick="window.deleteTx('${tx.id}')" class="text-xs font-medium text-rose-600 hover:text-rose-800 transition-colors">Xóa</button>
            </div>
          </div>
        </div>
      `;
  ...
  const renderTxList = (transactions) => {
    ...
    const emptyRow =
      '<tr><td colspan="6" class="p-4 text-center text-slate-500 text-sm">Không có giao dịch nào</td></tr>';
      ...
      if (legendEl) {
        ...
        legendEl.innerHTML =
          ...
          `;
            ...
            .join("") ||
          '<p class="text-slate-500">Chưa có dữ liệu chi tiêu.</p>';
      }
    } catch (error) {
      showToast(error.message, "error");
    }
  ...
  const initNotifications = () => {
    ...
    const emptyHtml =
      '<p class="px-4 py-6 text-center text-slate-500 text-sm">Không có thông báo nào.</p>';
  ...
  window.openEditTxModal = (txId) => {
    ...
    if (tx.type === "CHI") {
      document.getElementById("tx-type-chi")?.click();
    } else {
      document.getElementById("tx-type-thu")?.click();
    }
      ...
      try {
        ...
        if (currentTxType === "CHI") {
          try {
            ...
            if (b && b.exceeded) {
              showToast(
                `Bạn đã chi vượt ngân sách hạng mục "${b.category}"!`,
                "error",
              );
            } else if (b && b.warning) {
              showToast(
                `Sắp vượt ngân sách hạng mục "${b.category}" (đã dùng ${b.percentUsed}%).`,
                "error",
              );
            }
          } catch (err) {
            /* không lấy được ngân sách thì bỏ qua, không chặn luồng thêm giao dịch */
          }
        }
  ...
  const bindMiniModal = (modalId) => {
    ...
    modal
      .querySelectorAll(".close-mini-btn")
              ...
              datasets: [
                {
                  ...
                  backgroundColor: rows.map(
                    (r) => CATEGORY_CHART_COLORS[r.category] || "#64748b",
                  ),
                  borderWidth: 0,
                },
              ],
  ...
  const initSavingsGoals = () => {
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        if (!targetAmount || targetAmount <= 0) {
          showToast("Vui lòng nhập số tiền mục tiêu hợp lệ", "error");
          return;
        }
    ...
    document
      .getElementById("save-deposit-btn")
      ?.addEventListener("click", async () => {
    ...
    document.querySelectorAll("#settings-tabs [data-tab]").forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        document
          ...
          .forEach((p) =>
            p.classList.toggle("hidden", p.dataset.panel !== link.dataset.tab),
          );
      });
    });
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        if (newPassword !== confirmPassword) {
          showToast("Nhập lại mật khẩu mới không khớp", "error");
          return;
        }
  ...
  const initAdminPage = () => {
    ...
    const loadStats = async () => {
      try {
        ...
        document.getElementById("st-transactions").textContent =
        ...
        logsEl.innerHTML =
          ...
          <div class="px-5 py-3 flex items-center justify-between gap-3">
            ...
            .join("") ||
          '<p class="px-5 py-6 text-center text-slate-500 text-sm">Chưa có ai dùng AI.</p>';
      } catch (error) {
        showToast(error.message, "error");
      }
    };
    ...
    const renderUsers = async (q = "") => {
      try {
        ...
        usersTbody.querySelectorAll(".toggle-block-btn").forEach((btn) => {
          btn.addEventListener("click", async () => {
            const nextStatus =
              btn.dataset.status === "BLOCKED" ? "ACTIVE" : "BLOCKED";
            if (
              !confirm(
                `Bạn chắc chắn muốn ${nextStatus === "BLOCKED" ? "khóa" : "mở khóa"} tài khoản này?`,
              )
            )
              return;
            try {
              const res = await apiAdminSetUserStatus(
                btn.dataset.id,
                nextStatus,
              );
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        if (!Number.isFinite(dailyQuota) || dailyQuota < 1) {
          showToast("Hạn mức phải là số >= 1", "error");
          return;
        }
        ...
        try {
          ...
          showToast(
            `Đã lưu cấu hình AI (${res.model}, hạn mức ${res.dailyQuota}/ngày).`,
          );
      ...
      return `
        <div class="card card-hover p-5">
          ...
          <div class="mb-2 flex justify-between items-end">
            ...
            <div class="text-xs font-medium text-slate-500 bg-slate-100 px-2 py-1 rounded-md">
              Ngày ${item.deductDay || 1} hằng tháng
            </div>
          </div>
          <div class="mt-3 pt-3 border-t border-gray-50 flex justify-end">
